...
public class SarsCoV2 extends Molecule{
    ...
    private SarsCoV2(double[][][][] qttys, Diffuse diffuse) {
		super(qttys, diffuse);
	}
    ...
    public static SarsCoV2 getMolecule() {
    	return molecule;
    }

    public void degrade(int x, int y, int z) {
        //degrade(Constants.SarsCoV2_HALF_LIFE, x, y, z);
    ...
    protected boolean templateInteract(Interactable interactable, int x, int y, int z) {
        if (interactable instanceof Macrophage){//# or type(interactable) is Neutrophil: 
        	...
        	if(!(cell.getStatus() == Phagocyte.APOPTOTIC || cell.getStatus() == Phagocyte.NECROTIC || cell.getStatus() == Phagocyte.DEAD)) {
        		if(Util.activationFunction(this.get(x, y, z), Constants.Kd_Covid, Constants.UNIT_T) > Rand.getRand().randunif()) {
        			double qtty = this.dec(Constants.VIRAL_UPTAKE_RATE * this.get(x, y, z), x, y, z);
        			cell.setStatus(Phagocyte.ACTIVATING);
        		}
        	}
            return true; 
        }
        if (interactable instanceof Pneumocyte){
        	...
        	if(!(cell.getStatus() == Phagocyte.APOPTOTIC || cell.getStatus() == Phagocyte.NECROTIC || cell.getStatus() == Phagocyte.DEAD)) {
        		if(Util.activationFunction(this.get(x, y, z), Constants.Kd_Covid, Constants.UNIT_T) > Rand.getRand().randunif()) {
        			double qtty = this.dec(Constants.VIRAL_UPTAKE_RATE * this.get(x, y, z), x, y, z);
        			cell.setViralLoad(cell.getViralLoad() + qtty);
        			cell.setStatus(Phagocyte.ACTIVATING);
        		}
        	}else if(cell.getStatus() == Phagocyte.NECROTIC) {
        		this.inc(cell.getViralLoad(), x, y, z);
        	}
        ...
        if (interactable instanceof Neutrophil){
        	...
        	if(!(cell.getStatus() == Phagocyte.APOPTOTIC || cell.getStatus() == Phagocyte.NECROTIC || cell.getStatus() == Phagocyte.DEAD)) 
        		if(Util.activationFunction(this.get(x, y, z), Constants.Kd_Covid, Constants.UNIT_T) > Rand.getRand().randunif()) 
        			this.dec(Constants.VIRAL_UPTAKE_RATE * this.get(x, y, z), x, y, z);
        	return true; 
        }
